...
from web.handlers.base import BaseHandler, BaseWebSocketHandler, logger_web_handler
...
class SubscribeUpdatingHandler(BaseWebSocketHandler):
    ...
    def check_origin(self, origin: str) -> bool:
        ...
        if not domain:
            return host.lower() == origin
        return origin == domain or origin.endswith(f".{domain}") or host.lower() == origin
    ...
    async def update(self, userid):
        ...
        try:
                ...
                if int(time.time()) - int(repos["lastupdate"]) > 24 * 3600:
                    for repo in repos["repos"]:
                        await self.send_global_message(
                            {
                                "code": 1000,
                                "message": f"-----开始更新 {repo['reponame']} 模板仓库-----",
                            }
                        )
                        if repo["repoacc"]:
                            ...
                            elif config.subscribe_accelerate_url == "gh-proxy":
                                url = f"{repo['repourl'].replace('https://github.com/', 'https://gh-proxy.com/https://raw.githubusercontent.com/')}/{repo['repobranch']}"
                            else:
                        ...
                        async with aiohttp.ClientSession(
                            timeout=aiohttp.ClientTimeout(
                                total=config.connect_timeout * 10,
                                connect=config.connect_timeout * 5,
                            )
                        ) as session:
                            ...
                            async with session.get(
                                ...
                                if res.status == 200:
                                    ...
                                    logger_web_handler.info(
                                        "200 Get repo %s history file success!",
                                        repo["reponame"],
                                    )
                                    await self.send_global_message(
                                        {
                                            "code": 1000,
                                            "message": "tpls_history.json 文件获取成功",
                                        }
                                    )

                                else:
                                    logger_web_handler.error(
                                        "Get repo %s history file failed! Reason: %s open error!",
                                        repo["reponame"],
                                        hfile_link,
                                    )
                                    await self.send_global_message(
                                        {
                                            "code": 0,
                                            "message": f"tpls_history.json 文件获取失败, 原因: 打开链接 {hfile_link} 出错!",
                                        }
                                    )
                                    await self.send_global_message(
                                        {
                                            "code": 0,
                                            "message": f"HTTP 代码: {res.status}, 错误信息: {res.reason if res.reason else 'Unknown'}",
                                        }
                                    )
                                    fail_count += 1
                                    continue
                                        ...
                                        if har["content"] == "":
                                            ...
                                            async with session.get(
                                                ...
                                                if har_res.status == 200:
                                                    ...
                                                    logger_web_handler.error(
                                                        "Update %s public template %s failed! Reason: %s open error!",
                                                        repo["reponame"],
                                                        har["name"],
                                                        har_url,
                                                    )
                                                    await self.send_global_message(
                                                        {
                                                            "code": 0,
                                                            "message": f"模板: {har['name']} 更新失败, 原因: 打开链接 {har_url} 出错!",
                                                        }
                                                    )
                                                    await self.send_global_message(
                                                        {
                                                            "code": 0,
                                                            "message": f"HTTP 代码: {har_res.status}, 错误信息: {har_res.reason if har_res.reason else 'Unknown'}",
                                                        }
                                                    )
                                                    fail_count += 1
                                                    continue
                                        har["update"] = True
                                        await self.db.pubtpl.mod(
                                            tpl[0]["id"], **har, sql_session=sql_session
                                        )
                                        logger_web_handler.info(
                                            "Update %s public template %s success!",
                                            repo["reponame"],
                                            har["name"],
                                        )
                                        await self.send_global_message(
                                    ...
                                    if har["content"] == "":
                                        ...
                                        async with session.get(
                                            ...
                                            if har_res.status == 200:
                                                ...
                                                logger_web_handler.error(
                                                    "Add %s public template %s failed! Reason: %s open error!",
                                                    repo["reponame"],
                                                    har["name"],
                                                    har_url,
                                                )
                                                await self.send_global_message(
                                                    {
                                                        "code": 0,
                                                        "message": f"模板: {har['name']} 添加失败, 原因: 打开链接 {har_url} 出错!",
                                                    }
                                                )
                                                await self.send_global_message(
                                                    {
                                                ...
                                                fail_count += 1
                                                continue
                                    await self.db.pubtpl.add(
                                        har, sql_session=sql_session
                                    )
                                    logger_web_handler.info(
                                        "Add %s public template %s success!",
                                        repo["reponame"],
                                        har["name"],
                                    )
                                    await self.send_global_message(
                                        {
                                            "code": 1000,
                                            "message": f"模板: {har['name']} 添加成功",
                                        }
                                    )
                        await self.send_global_message(
                            {
                                "code": 1000,
                                "message": f"-----更新 {repo['reponame']} 模板仓库结束-----",
                            }
                        )
            success = True
        except Exception as e:
            ...
            if msg.endswith("\r\n"):
                msg = msg[:-2]
            logger_web_handler.error(
                "UserID: %s update Subscribe failed! Reason: %s",
                userid,
                msg,
                exc_info=config.traceback_print,
            )
            await self.send_global_message(
                {"code": 0, "message": f"更新失败, 原因: {msg}"}
            )
            ...
            if success:
                if fail_count == 0:
                    await self.close_all(
                        1000, "Update success, please refresh your browser."
                    )
                else:
                    await self.close_all(
                        4001,
                        f"Update success, but {fail_count} templates update failed.",
                    )
            else:
                await self.close_all(
                    4006, "Update failed, please check failure reason."
                )
        except Exception as e:
            logger_web_handler.error(
                "UserID: %s update Subscribe or close connection failed! Reason: %s",
                userid,
                e,
                exc_info=config.traceback_print,
            )
    ...
    def on_close(self):
        for userid, ws in list(SubscribeUpdatingHandler.users.items()):
            if ws == self:
                SubscribeUpdatingHandler.users[userid].close(
                    1000, "client close connection"
                )
    ...
    async def open(self, userid):  # pylint: disable=arguments-differ,invalid-overridden-method
        ...
        if user["id"] != int(userid):
            self.close(
                4403,
                "Forbidden: userid not match with cookie, please check your cookie or login again",
            )
            return
        ...
        if (
            not adminflg
            and len(SubscribeUpdatingHandler.users)
            >= config.websocket.max_connections_subscribe
        ):
        ...
        if len(SubscribeUpdatingHandler.users) == 0:
            if SubscribeUpdatingHandler.updating and (
                int(time.time()) - SubscribeUpdatingHandler.updating_start_time > 60
            ):
                SubscribeUpdatingHandler.updating = False
        elif userid in SubscribeUpdatingHandler.users:
            SubscribeUpdatingHandler.users[userid].close(
                1001, "Another client login, close this connection"
            )
            SubscribeUpdatingHandler.users.pop(userid)
        ...
        try:
            ...
            async with self.db.transaction() as sql_session:
                if (user["id"] == int(userid)) and (user["role"] == "admin"):
                    ...
                    if op == "clear":
                        for pubtpl in await self.db.pubtpl.list(
                            fields=("id",), sql_session=sql_session
                        ):
        ...
        try:
            ...
            if (user["id"] == int(userid)) and (user["role"] == "admin"):
                ...
                if (
                    ...
                    repos = json.loads(
                        (await self.db.site.get(1, fields=("repos",)))["repos"]
                    )
                    tmp = repos["repos"]
                    inflg = False
                    ...
                    if inflg:
                        ...
                        repos["lastupdate"] = 0
                        await self.db.site.mod(
                            1, repos=json.dumps(repos, ensure_ascii=False, indent=4)
                        )
                else:
...
class GetReposInfoHandler(BaseHandler):
    ...
    async def post(self, userid):
        try:
            ...
            await self.render(
                "utils_run_result.html", log=str(e), title="获取信息失败", flg="danger"
            )
            return
...
class UnsubscribeReposHandler(BaseHandler):
    ...
    async def get(self, userid):
        try:
            ...
            await self.render(
                "utils_run_result.html", log=str(e), title="打开失败", flg="danger"
            )
            return
