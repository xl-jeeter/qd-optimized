...
class TaskNewHandler(BaseHandler):
    async def get(self):
        ...
        if user:
            tpls += sorted(
                await self.db.tpl.list(userid=user["id"], fields=fields, limit=None),
                key=lambda t: -t["id"],
            )
        if tpls:
            tpls.append({"id": 0, "sitename": "-----公开模板-----"})
        tpls += sorted(
            await self.db.tpl.list(userid=None, public=1, fields=fields, limit=None),
    ...
    async def post(self, taskid=None):
        ...
        async with self.db.transaction() as sql_session:
            ...
            if not taskid:
                env = await self.db.user.encrypt(
                    user["id"], env, sql_session=sql_session
                )
                taskid = await self.db.task.add(
                    tplid, user["id"], env, sql_session=sql_session
                )
                ...
                if task["retry_interval"] == retry_interval or (
                    retry_interval == "" and task["retry_interval"] is None
                ):
                    retry_interval_modified = False

                init_env = await self.db.user.decrypt(
                    user["id"], task["init_env"], sql_session=sql_session
                )
                init_env.update(env)
                init_env = await self.db.user.encrypt(
            ...
            if "New_group" in envs:
                await self.db.task.mod(
                    taskid, _groups=target_group, sql_session=sql_session
                )

            if isinstance(retry_count, int) and -1 <= retry_count:
                await self.db.task.mod(
                    taskid, retry_count=retry_count, sql_session=sql_session
                )

            if retry_interval_modified:
                if retry_interval:
                    await self.db.task.mod(
                        taskid, retry_interval=retry_interval, sql_session=sql_session
                    )
                else:
                    await self.db.task.mod(
...
class TaskEditHandler(TaskNewHandler):
    ...
    async def get(self, taskid):  # pylint: disable=W0221
        ...
        task["init_env"] = await self.db.user.decrypt(
            user["id"], (await self.db.task.get(taskid, ("init_env",)))["init_env"]
        )
...
class TaskRunHandler(BaseHandler):
    ...
    async def post(self, taskid):
        ...
        async with self.db.transaction() as sql_session:
            ...
            env = dict(
                variables=await self.db.user.decrypt(
            ...
            try:
                ...
                if not url:
                    # 如果任务编辑页没有设置代理，使用默认方式执行
                    # 在执行过程中，env['variables']['_proxy'] 可能会被修改
                    new_env, _ = await self.fetcher.do_fetch(fetch_tpl, env)
                else:
                    # 使用任务编辑页设置的初始代理开始执行
                    # 但在执行过程中，可能会通过修改 env['variables']['_proxy'] 来改变代理
                    proxy = {
                        "scheme": url["scheme"],
                        "host": url["host"],
                        "port": url["port"],
                ...
                # 在任务完成后，new_env['variables'] 中将包含最后使用的 _proxy 值
                ...
                try:
                    ...
                    safe_log = xhtml_escape(logtmp).replace("\\r\\n", "<br>")
                    await self.finish(
                        '<h1 class="alert alert-danger text-center">运行失败</h1><div class="showbut well autowrap" id="errmsg">%s<button class="btn hljs-button" data-clipboard-target="#errmsg" >复制</button></div>'
                        % safe_log
                    )
                except StreamClosedError as e:
                    logger_web_handler.error(
                        "stream closed error: %s", e, exc_info=config.traceback_print
                    )
            ...
            try:
                ...
                safe_log = xhtml_escape(logtmp).replace("\\r\\n", "<br>")
                await self.finish(
                    '<h1 class="alert alert-success text-center">运行成功</h1><div class="showbut well autowrap" id="errmsg"><pre>%s</pre><button class="btn hljs-button" data-clipboard-target="#errmsg" >复制</button></div>'
                    % safe_log
                )
            except StreamClosedError as e:
                logger_web_handler.error(
...
class TaskLogHandler(BaseHandler):
    ...
    async def get(self, taskid):
        # user = self.current_user
        task = self.check_permission(
            await self.db.task.get(taskid, fields=("id", "tplid", "userid", "disabled"))
        )
...
class TotalLogHandler(BaseHandler):
    ...
    async def get(self, userid, days):
        ...
        if userid == str(user["id"]):
            for task in await self.db.task.list(
                userid, fields=("id", "tplid", "note"), limit=None
            ):
                tpl = await self.db.tpl.get(
                    task["tplid"],
...
class TaskLogSuccessDelHandler(BaseHandler):
    ...
    async def get(self, taskid):
        ...
        async with self.db.transaction() as sql_session:
            ...
            await self.db.task.mod(taskid, success_count=0, sql_session=sql_session)
...
class TaskLogFailDelHandler(BaseHandler):
    ...
    async def get(self, taskid):
        ...
        async with self.db.transaction() as sql_session:
            ...
            await self.db.task.mod(taskid, failed_count=0, sql_session=sql_session)
...
class TaskSetTimeHandler(BaseHandler):
    ...
    async def get(self, taskid):
        ...
        await self.render(
            "task_setTime.html", task=task, ontime=ontime, today_date=today_date
        )
    ...
    async def post(self, taskid):
        ...
        await self.render(
            "utils_run_result.html", log=log, title="设置成功", flg="success"
        )
        return
...
class TaskGroupHandler(BaseHandler):
    ...
    async def get(self, taskid):
        ...
        await self.render(
            "task_setgroup.html", taskid=taskid, _groups=_groups, groupNow=group_now
        )
    ...
    async def post(self, userid):  # pylint: disable=unused-argument
        try:
            ...
            if "taskids" in body_arguments:
                ...
                await self.finish(
                    '<h1 class="alert alert-success text-center">操作成功</h1>'
                )
            raise Exception("taskids not found!")
        except Exception as e:
            logger_web_handler.error(
                "TaskID: %s delete failed! Reason: %s",
                taskid,
                str(e).replace("\\r\\n", "\r\n"),
                exc_info=config.traceback_print,
            )
