...
class Fetcher(object):
    ...
    def build_request(
        ...
        req = httpclient.HTTPRequest(
            ...
            validate_cert=bool(strtobool(os.getenv("VALIDATE_CERT", "True"))),
    ...
    def parse(self, tpl):
        ...
        if stmt_stack:
            raise Exception(f"Unclosed template blocks: {[s['type'] for s in stmt_stack]}")
    ...
    async def do_fetch(
        ...
        for i, block in enumerate(self.parse(tpl)):
            if request_limit <= 0:
                ...
                try:
                    if isinstance(_from_var, str) and (
                        _from_var.startswith("list(")
