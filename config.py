...
_cookie_secret_env = os.getenv("COOKIE_SECRET")
_aes_key_env = os.getenv("AES_KEY")
if not _cookie_secret_env or _cookie_secret_env == "binux":
    import logging as _logging
    _logging.warning(
        "SECURITY WARNING: COOKIE_SECRET is not set or using default value 'binux'. "
        "Set COOKIE_SECRET environment variable to a random string for production use!"
    )
if not _aes_key_env or _aes_key_env == "binux":
    import logging as _logging
    _logging.warning(
        "SECURITY WARNING: AES_KEY is not set or using default value 'binux'. "
        "Set AES_KEY environment variable to a random string for production use!"
    )
cookie_secret = hashlib.sha256(
    os.getenv("COOKIE_SECRET", "binux").encode("utf-8")
).digest()  # Cookie 加密密钥, 强烈建议修改
# PBKDF2 迭代次数 - OWASP 2023 推荐至少 600,000 次 (SHA-256)
# 注意: 修改此值后需要用户重新设置密码
